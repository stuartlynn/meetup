...
def clean_user_locations(userLocations: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    users_with_duplicates = (userLocations.groupby("user_id").count() > 1).sum().latitude
    if users_with_duplicates > 0:
        logWarning(f"""Found [bold white]{users_with_duplicates}[/bold white] users who had multiple entries in the input file. 
...
def load_user_locations(filePath:str, latCol: Optional[str] ='latitude', lngCol:Optional[str]='longitude', userIdCol: Optional[str] = 'user_id') -> gp.GeoDataFrame:
    """
    ...
    assert userIdCol in userLocations.columns, f"The supplied user_id column: {userIdCol} is not present in the supplied input file"
    assert latCol in userLocations.columns, f"The supplied latitude column: {latCol} is not present in the supplied input file"
    assert lngCol in userLocations.columns, f"The supplied longitude column: {lngCol} is not present in the supplied input file"
    userLocations.rename(columns= {lngCol: "longitude", latCol: "latitude", userIdCol:"user_id"}, inplace=True)

    userLocations = clean_user_locations(userLocations)

    userLocations = gp.GeoDataFrame(userLocations, geometry= gp.points_from_xy(userLocations[lngCol], userLocations[latCol]), crs="epsg:4326")
...
def snap_points_to_street_network(points: gp.GeoDataFrame, network: MultiDiGraph ) -> gp.GeoDataFrame:
    """
    ...
    snappedPoints = gp.points_from_xy([node['x'] for node in nodes], [node['y'] for node in nodes])
...
def generate_group_centroids(clusteredUsers : gp.GeoDataFrame)->gp.GeoDataFrame:
    ...
    """
    centroids = GeoDataFrame(clusteredUsers.groupby('label').geometry.apply(lambda points: MultiPoint(points.values).centroid).reset_index())
...
def get_network_graph( bounds: tuple[float,float,float,float], mode='bike', crs='epsg:4326') -> MultiDiGraph:
    """
    ...
    if(network is None):
        network = ox.graph_from_bbox(bounds[3], bounds[1], bounds[2], bounds[0],network_type=mode)
        network = ox.project_graph(network,crs)
        RunCache.set_network("cycle_network", network)
    return network 

def generate_cluster_convex_hull(df:GeoDataFrame, groupBy:str="label", geometryCol:str="geometry"):
    """
